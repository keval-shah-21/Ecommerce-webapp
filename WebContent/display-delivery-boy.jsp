...
<jsp:include page="/DisplayUser"/>
	<%List<User> userList = new ArrayList<User>(); %>
        ...
        <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
            ...
            <%if(obj.getRole().equalsIgnoreCase("Delivery Boy")){ %>
                <tr>
                    <td><%=obj.getFname() %></td>
                    <td><%=obj.getLname() %></td>
                    <td><%=obj.getEmail() %></td>
                   ...
                   <% if(obj.getContact() !=null && obj.getContact().isEmpty()) {%> 
                    <td>NA</td>
                    <%} %>
                    <td><%=obj.getAddress1() %></td>
                    <td><%=obj.getSalary() %></td>
                     <td><a style="color:white;"href="EditDeliveryBoy?id=<%=obj.getCustomerId()%>"><button class="btn btn-primary" style="color:white;"> <i class="fa fa-pen"></i>  </button></a></td>
                    
                    <td><button class="btn btn-primary"  style="background-color:red;" data-toggle="modal" data-target="#exampleModalCenter"
													onclick="getdid(<%=obj.getCustomerId()%>)"><i class="fa fa-trash" > </i></button></td>
